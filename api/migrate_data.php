...
function getCategoryQuestionCount($categoryId) {
    $countData = fetchJson(OTDB_API_BASE_URL . 'api_count.php?category=' . $categoryId);
    if (isset($countData['category_question_count'])) {
        return $countData['category_question_count']['total_question_count'];
    }
    return 0;
}
...
function migrate_data() {
    ...
    if (isset($categoriesData['trivia_categories'])) {
        ...
        foreach ($categories as $category) {
            ...
            $totalQuestionsInCategory = getCategoryQuestionCount($category['id']);
            echo "Total questions in this category: $totalQuestionsInCategory\n";

            $totalQuestionsMigrated = 0;

            if ($totalQuestionsInCategory > 0) {
                $remainingQuestions = $totalQuestionsInCategory;
                while ($remainingQuestions > 0) {
                    $amountToFetch = min($remainingQuestions, QUESTIONS_PER_REQUEST);
                    $apiUrl = OTDB_API_BASE_URL . 'api.php?amount=' . $amountToFetch . '&category=' . $category['id'] . '&token=' . $token;
